...
class Level:
    ...
    departure:float=0.0; touches:int=0; grade:str="C"; in_zone:bool=False
...
class Engine:
    ...
    def departure_of(s,price,at_i,n=3):
        """FIX: spec ne ye sirf LAUNCH ke liye define kiya tha. Har level ke liye
        wahi cheez maayne rakhti hai — price is jagah se kitni tezi se bhaagi."""
        a=s.atr(20) or 20
        j=at_i+n
        if j>=len(s.c1): return 0.0
        return abs(s.c1[j].c-price)/a
    ...
    def swings(s,arr,k,tf):
        ...
        if all(H[i]>H[i-j] for j in range(1,k+1)) and all(H[i]>H[i+j] for j in range(1,k+1)):
            s.add(Level(f"T{tf}H{arr[i].i}","TURN","resistance",arr[i].i,tf,arr[i].body_top,arr[i].h,
                        s.departure_of(arr[i].h,arr[i].i)))
        ...
        if all(L[i]<L[i-j] for j in range(1,k+1)) and all(L[i]<L[i+j] for j in range(1,k+1)):
            s.add(Level(f"T{tf}L{arr[i].i}","TURN","support",arr[i].i,tf,arr[i].body_bot,arr[i].l,
                        s.departure_of(arr[i].l,arr[i].i)))
    ...
    def breaks(s):
        ...
        for lv in list(s.levels.values()):
            ...
            if broke and lv.kind!="ANCHOR" and lv.grade in ("A","B"):   # FIX 5
    ...
    def touches_and_death(s):
        ...
        for lv in s.levels.values():
            ...
            # FIX: ek TEST aur ek MINUTE alag hain. Touch tabhi ginte hain jab
            # price zone se BAAHAR ja kar WAPAS aaye — warna 3 minute mandrana
            # 3 touches ban jaata hai aur level bina test hue mar jaata hai.
            sep = 0.6*a
            inside = (c.l <= lv.zhi+2) and (c.h >= lv.zlo-2)
            if inside and not lv.in_zone:
                lv.touches += 1; lv.in_zone = True
            elif not inside and (c.l > lv.zhi+sep or c.h < lv.zlo-sep):
                lv.in_zone = False
    ...
    def grade(s):
        ...
        for lv in s.levels.values():
            ...
            if lv.kind=="ANCHOR": sc+=1   # FIX: poore session me bana = clean formation
    ...
    def guards(s):
        ...
        pos_in_15 = (m-T0)%15
        s.in_htf_tail = pos_in_15 >= 15-P["htf_close_buffer_min"]
        # FIX: guard ki wajah = "HTF candle aakhri minutes me shakal palatti hai".
        # Setup B usi palat ko trade karta hai. Guard ka reason usspe lagta hi nahi.
        # To tail me sirf CONTINUATION setups block honge (A/C), Setup B nahi.
        a=s.atr()
        ...
        for back in (0,1,2):
            ...
            for lv in s.levels.values():
                ...
                if lv.side in ("support","axis"):
                    ...
                    # FIX 6: pullback entry — entry ko extreme ke 1.2xATR ke andar rakho
                    a2=s.atr() or 20
                    cap_entry = sw.l + 0.85*a2
                    entry = cur.c if cur.c <= cap_entry else cap_entry
                    return dict(setup="B_sweep_reclaim",dir="long",level=lv,extreme=sw.l,
                        entry=entry, ev=dict(entry_mode=("reclaim_close" if entry==cur.c else "pullback"),
                        entry_reclaim_close=round(cur.c,1), entry_pullback=round(cap_entry,1),
    ...
    def on_candle(s,o,h,l,c):
        ...
        if md=="BLOCKED":
            ...
            cand = s.setup_B()
            if not cand and not getattr(s,"in_htf_tail",False): cand = s.setup_A()
            if cand and getattr(s,"in_htf_tail",False) and cand["setup"][0]!="B":
                cand=None
            if not cand:
